...
def menu_v2(
    ...
    for c in choices:
        ...
        if label_attrs:
            ...
            button = Button(c)
        connect_signal(button, "click", callback, user_args=(c))
        if as_table:
            body.append(button)
        else:
            body.append(AttrMap(button, attr_map=None, focus_map="reversed"))
...
def routing_menu(
    frame: Frame, nav: list[tuple], title: str, routes: list[Route]
) -> ListBox:
...
def show_new_screen(frame: Frame, new_screen_callable, nav: list[tuple], *args) -> None:
