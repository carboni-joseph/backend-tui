...
def customer_chosen(frame: urwid.Frame, choice: SCACustomer, button) -> None:
    ...
    new_menu = partial(
        menu, frame, new_title, action_menu, choice.adp_objs, ["adp_alias"]
    )
...
def program_type_selected(
    ...
    try:
        ...
        response = (
            f"{program_type.type_selected} program downloaded "
...
def show_coil_status_change_menu(
    customer: ADPCustomer, frame: urwid.Frame, coil: Coil, button
) -> None:
    title = (
    ...
    new_menu = partial(
        menu, frame, title, partial(update_coil_status, customer, coil), choices=Stage
    )
    show_new_screen(frame, new_menu, NAV_STACK)


def show_ah_status_change_menu(
    customer: ADPCustomer, frame: urwid.Frame, ah: AH, button
) -> None:
    title = (
        f"{customer.adp_alias}\n "
    ...
    new_menu = partial(
        menu, frame, title, partial(update_ah_status, customer, ah), choices=Stage
    )
    show_new_screen(frame, new_menu, NAV_STACK)


def update_coil_status(
    customer: ADPCustomer, coil: Coil, frame: urwid.Frame, new_stage: str, button
) -> None:
...
def update_ah_status(
    customer: ADPCustomer, ah: AH, frame: urwid.Frame, new_stage: str, button
) -> None:
...
def add_new_model(submit_method: Callable, customer: ADPCustomer) -> urwid.ListBox:
    ...
    submit = urwid.Button(
        "Submit", on_press=partial(submit_method, customer), user_data=edit
    )
...
def do_model_lookup(customer: ADPCustomer):
    ...
    return urwid.ListBox(
        [user_input, urwid.AttrMap(submit, None, focus_map="reversed")]
    )
...
def submit_model(
    product_type_method: Callable, customer: ADPCustomer, user_input: urwid.Edit
) -> None:
...
def remove_rating(
    rating: Rating, frame: urwid.Frame, confirm: Literal["Yes", "No"], button=None
) -> None:
    if confirm == "No":
        go_back()
        return
...
def confirm_rating_delete(frame: urwid.Frame, rating: Rating, button) -> None:
    new_menu = partial(
        menu, frame, "Delete?", partial(remove_rating, rating), ["Yes", "No"]
    )
    show_new_screen(frame, new_menu, NAV_STACK)
...
def action_chosen(
    customer: ADPCustomer, frame: urwid.Frame, choice: str, button
) -> None:
